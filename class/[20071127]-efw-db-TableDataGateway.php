...
class EFW_DB_TableDataGateway {
	...
	public function setFieldList ($params) {
		$this->_field = $params;
	}
	
	/**
	 * $conditions = 23;
	 * $conditions = "name = 'eason007'";
	 * $conditions = array("name <> 'eason007'", "sex = 'male'");
	...
	*/
	public function setConditions ($params) {
		...
		if (is_array($params)){
			foreach($params as $key => $val){
				if (is_numeric($key)){
					if (is_numeric($val)){
						...
						$conditions.= " AND ".$this->sqlEncode($val);
					}
				}
				else{
			...
			if (is_numeric($params)){
				$conditions = " AND `".$this->primaryKey."` = ".$params;
			}
			else{
				$conditions = " AND ".$this->sqlEncode($params);
			}
		}
	 ...
	 * 对查询结果设定排序条件
	 * 输入参数支持字符串及数组形式，如：
	 * $a->setOrders("UserID DESC, UserName ASC");
	 * $a->setOrders(array("UserID" => "DESC", "UserName" => "ASC"));
	 *
	...
	public function setLimit ($params) {
		$this->_limit = $this->getLimit($params);
	}
	...
	public function select ($link = "", $isExecute = true, $isCount = false) {
		...
		$c_sql.= " WHERE 1=1".$this->_conditions;
		$c_sql.= $this->other;
		$c_sql.= $this->order;
	...
	private function getLimit ($params) {
		...
		if (is_array($params)){
			return " LIMIT ".$params["offset"] * $params["length"].",".$params["length"];
		}
		else{
			return " LIMIT ".$params;
		}
